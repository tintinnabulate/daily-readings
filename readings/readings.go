...
// PrintReflections : do it
func PrintReflections(out io.Writer) {
	...
	fmt.Fprintln(out, today.Month, today.Day)
	fmt.Fprintln(out)
	fmt.Fprintln(out, today.Quotation)
	fmt.Fprintln(out, today.Citation)
	fmt.Fprintln(out)
	fmt.Fprintln(out, today.Reading)
}
